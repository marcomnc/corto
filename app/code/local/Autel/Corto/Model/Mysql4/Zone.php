 ...
 * @author marcoma
 */
class Autel_Corto_Model_Mysql4_Zone extends Mage_Core_Model_Mysql4_Abstract
{
    
    protected function _construct()
    {
        $this->_init('autelcorto/zone', 'entity_id');
    }

    public function getStoreLabels($zoneId)
    {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('autelcorto/zonelabel'), array('store_id', 'label'))
    ...
    {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('autelcorto/zonelabel'), array('label'))
            ->where('zone_id=?', $zoneId)
    ...
    public function saveStoreLabels($zoneId, $labels){
        $delete = array();
        $save = array();
        $table = $this->getTable('autelcorto/zonelabel');
