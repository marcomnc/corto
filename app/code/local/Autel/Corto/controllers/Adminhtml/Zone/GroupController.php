 ...
 *
 */
class Autel_Corto_Adminhtml_Zone_GroupController extends Mage_Adminhtml_Controller_Action 
{
...
//        } else {
            ...
            $priceZoneBlock = $this->getLayout()->createBlock('autelcorto/adminhtml_zone_group_group');
...
//        }
    ...
    public function editAction() {
        ...
        $model  = Mage::getModel('autelcorto/zonegroup');

        if ($id) {  
            $model->load($id);
            if (!$model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('autelcorto')->__('Il gruppo selezionato non esiste'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        Mage::register('autelcorto_zone_group', $model);
        

        $this->_addContent($this->getLayout()->createBlock('autelcorto/adminhtml_zone_group_edit'));
    ...
    {
        if ($id = $this->getRequest()->getParam('entity_id')) {
            $zone = Mage::getModel('autelcorto/zonegroup')->load($id);
            $name = $zone->getName();
            try {
                $zone->delete();
