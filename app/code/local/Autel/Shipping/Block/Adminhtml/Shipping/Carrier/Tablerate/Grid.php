 ...
 */
class Autel_Shipping_Block_Adminhtml_Shipping_Carrier_Tablerate_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    /**
     * Website filter
     *
    ...
    public function setCarrireCode($carrireCode) {
        $this->_carrireCode = $carrireCode;
        return $this;
    }
    
    public function getCarrierCode(){
        return $this->_carrireCode;
    }    
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = Mage::getResourceModel('autelshipping/carrier_auteltablerate_collection');
        $collection->setConditionFilter($this->getConditionName())
            ->setWebsiteFilter($this->getWebsiteId())
            ->setCarrireFilter($this->getCarrierCode());
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('surcharge', array(
            'header'    => Mage::helper('autelshipping')->__('Fuel Surcharge %'),
            'index'     => 'surcharge',
        ));

        $this->addColumn('insurance', array(
            'header'    => Mage::helper('autelshipping')->__('Insurance %'),
            'index'     => 'insurance',
        ));
        
        $this->addColumn('advance', array(
            'header'    => Mage::helper('autelshipping')->__('Advance Customs Documents price'),
            'index'     => 'advance',
        ));
                
        $this->addColumn('cites', array(
            'header'    => Mage::helper('autelshipping')->__('Cites Price'),
            'index'     => 'cites',
        ));
                        
       $this->addColumn('cites_customs', array(
            'header'    => Mage::helper('autelshipping')->__('Cites Custom Price'),
            'index'     => 'cites_customs',
        ));             
