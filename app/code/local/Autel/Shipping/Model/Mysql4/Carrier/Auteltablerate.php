 ...
 */
class Autel_Shipping_Model_Mysql4_Carrier_Auteltablerate extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    protected $_tableIdx = "autel_dhl_shipping";

    protected function _construct()
    {
        $this->_init('autelshipping/auteltablerate', 'pk');        
    }
    
    public function setCarrierCode($carrierCode) {
        $this->_carrierCode = strtoupper($carrierCode);
        $this->_tableIdx = "autel_" . strtolower($this->_carrierCode) . "_shipping";
        return $this;        
    }
    ...
    public function getRate(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        $select = $adapter->select()
            ...
            ->where('carrier_code = ?', $this->_carrierCode)
            ->where('(website_id = :website_id) OR (website_id = 0)')                
            ->order(array('website_id DESC', 'dest_country_id DESC', 'dest_region_id DESC', 'dest_zip DESC', 'condition_value DESC'))
            ->limit(1);
    ...
    {
        if (empty($_FILES['groups']['tmp_name'][$this->_tableIdx]['fields']['import']['value'])) {
            return $this;
        }

        $csvFile = $_FILES['groups']['tmp_name'][$this->_tableIdx]['fields']['import']['value'];
        ...
        //}
        $this->_importConditionName = Autel_Shipping_Model_Carrier_Auteltablerate::SHIPPING_CONDITION;
        ...
        try {
            ...
            $condition = array(
                'carrier_code = ?'   => $this->_carrierCode,
    ...
    protected function _getImportRow($row, $rowNumber = 0)
    {
        ...
        // validate surcharge
        $surcharge = $this->_parseDecimalValue($row[5]);
        if ($surcharge === false) {
            $this->_importErrors[] = Mage::helper('autelshipping')->__('Invalid surcharge Percentage "%s" in the Row #%s.', $row[5], $rowNumber);
            return false;
        }

        // validate insurance
        $insurance = $this->_parseDecimalValue($row[6]);
        if ($surcharge === false) {
            $this->_importErrors[] = Mage::helper('autelshipping')->__('Invalid insurance Percentage "%s" in the Row #%s.', $row[6], $rowNumber);
            return false;
        }

        // validate advance
        $advance = $this->_parseDecimalValue($row[7]);
        if ($surcharge === false) {
            $this->_importErrors[] = Mage::helper('autelshipping')->__('Invalid advance  custom cost "%s" in the Row #%s.', $row[7], $rowNumber);
            return false;
        }

        // validate cites
        $cites = $this->_parseDecimalValue($row[8]);
        if ($cites === false) {
            $this->_importErrors[] = Mage::helper('autelshipping')->__('Invalid cites cost "%s" in the Row #%s.', $row[8], $rowNumber);
            return false;
        }

        // validate cites_customs
        $cites_custom = $this->_parseDecimalValue($row[9]);
        if ($cites_custom === false) {
            $this->_importErrors[] = Mage::helper('autelshipping')->__('Invalid cites custom cost "%s" in the Row #%s.', $row[9], $rowNumber);
            return false;
        }
        ...
        return array(
            $this->_carrierCode,        //Codice Vettore
    ...
    {
        if (!empty($data)) {
            $columns = array('carrier_code', 'website_id', 'dest_country_id', 'dest_region_id', 'dest_zip',
                'condition_name', 'condition_value', 'price', 'surcharge', 'insurance', 'advance', 'cites',
                'cites_custom');
