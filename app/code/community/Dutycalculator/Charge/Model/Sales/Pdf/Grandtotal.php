...
class Dutycalculator_Charge_Model_Sales_Pdf_Grandtotal extends Mage_Sales_Model_Order_Pdf_Total_Default
{
    ...
    {
		if ($this->getOrder()->getDeliveryDutyType() == Dutycalculator_Charge_Helper_Data::DC_DELIVERY_TYPE_DDU || $this->getOrder()->getFailedCalculation())
		{
		...
		else
		{
			...
			$amountExclTax = ($amountExclTax > 0) ? $amountExclTax : 0;
			$amountExclTax = $this->getOrder()->formatPriceTxt($amountExclTax);
			$tax = $this->getOrder()->formatPriceTxt($this->getSource()->getTaxAmount() + $this->getSource()->getImportDutyTax());
			$fontSize = $this->getFontSize() ? $this->getFontSize() : 7;
