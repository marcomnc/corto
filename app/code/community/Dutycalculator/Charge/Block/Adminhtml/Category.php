...
class Dutycalculator_Charge_Block_Adminhtml_Category extends Mage_Adminhtml_Block_Catalog_Form_Renderer_Fieldset_Element
{
	public function getElementHtml()
	{
		...
		if ($product->getId())
		{
			...
			$formattedProductParams = '?product_name=' . urlencode($product->getName()) . '&product_description=' . urlencode($description) . '&product_url=' . urlencode($product->getProductUrl()) . '&product_price=' . number_format($product->getPrice(), 2);
